...
MAX_MAP_COUNT={{elasticsearch_vm_max_map_count}}
LOG_DIR={{elasticsearch_log_folder}}
CONF_DIR={{elasticsearch_conf_path}}
DATA_DIR={{elasticsearch_data_folder}}
...
start() {
    ...
    daemon --user $ES_USER --pidfile $pidfile $exec -p $pidfile -d -Epath.conf=$CONF_DIR
    #-Ees.default.path.home=$ES_HOME -Ees.default.path.logs=$LOG_DIR -Ees.default.path.data=$DATA_DIR
    retval=$?
    echo
